...
const defaultVolumeRoot = "/mnt/volumes"	// different from volume.DefaultDockerRootDirectory, used by linked mount path (ALiOssVolumeDriver.mount). 
											// It's different from "ossfsRoot", I don't know why it is needed, maybe I will retain just one of the two vars later.
var (
...
func main() {
	...
	var Usage = func() {
		...
		fmt.Printf("Describe: a docker volume plugin driver for oss of aliyun based on ossfs \n")
		fmt.Printf("Author: Loongel\n")
		fmt.Printf("Note: this driver rely on ossfs driver of aliyun, install ossfs like this:\n" +
		    "	read https://github.com/aliyun/ossfs Section：install\n " +"\n"+
	...
	var Excute = func(dfs map[string]OssDef){
		...
		if *debug {
			...
			driver := NewALiOssVolumeDriver(mount, drvn, dbg)
